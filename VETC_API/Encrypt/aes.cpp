...
int AES::setKey(aes_context * ctx, uint8 * key, int nbits)
{
	int i;
	uint32 *RK, *SK;
...
void AES::encrypt(aes_context * ctx, uint8 input[16], uint8 output[16])
{
...
void AES::decrypt(aes_context * ctx, uint8 input[16], uint8 output[16])
{
	uint32 *RK, X0, X1, X2, X3, Y0, Y1, Y2, Y3;
